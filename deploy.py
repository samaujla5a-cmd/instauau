...
def _validate_env():
    ...
    required = {
        "KIE_API_KEY":        "https://kie.ai → sign up free",
        "RAP_IG_TOKEN":       "Meta Developer Portal → your app → Instagram",
        "RAP_IG_USER_ID":     "Meta Developer Portal → your app → Instagram",
        "BRAINROT_IG_TOKEN":  "Meta Developer Portal → your app → Instagram",
        "BRAINROT_IG_USER_ID":"Meta Developer Portal → your app → Instagram",
        "MODEL_IG_TOKEN":     "Meta Developer Portal → your app → Instagram",
        "MODEL_IG_USER_ID":   "Meta Developer Portal → your app → Instagram",
    }
    missing = [k for k, _ in required.items() if not os.environ.get(k)]
    if missing:
        for k in missing:
            logger.error(f"Missing required env var: {k}  (see: {required[k]})")
        # Warn but don't exit — allow partial operation (e.g. only some channels)
        logger.warning(f"{len(missing)} required env vars missing — some channels may fail")
