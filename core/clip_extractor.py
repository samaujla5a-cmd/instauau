...
╔══════════════════════════════════════════════════════════╗
║         CLIP EXTRACTOR                                   ║
║         AI-powered best moment detection for Shorts     ║
╚══════════════════════════════════════════════════════════╝
"""
...
# FIX: use the shared Groq→Gemini client instead of calling Anthropic directly
...
Return ONLY valid JSON array."""
    ...
    for i, clip in enumerate(clips[:4]):
        ...
        logger.info(f"  📌 Clip {clip['clip_number']}: {start:.0f}s–{end:.0f}s ({clip['section']}) - {clip['energy_level']} energy")
...
def get_clip_metadata(concept: dict, clip: dict, trending_tags: list[str]) -> dict:
    """Build upload-ready metadata for each short/reel."""
    # FIX: was importing non-existent build_hashtag_string
    from research.trend_researcher import build_instagram_metadata

    base_caption = clip["caption_hook"]
    meta     = build_instagram_metadata(concept)
    hashtags = " ".join(meta.get("hashtags", trending_tags))

    return {
        "yt_title":       f"{clip['title']} 🌿 {concept['title']} #Shorts",
        "yt_description": f"{base_caption}\n\n{concept['title']} - Full song on channel 🔔\n\n{hashtags}",
