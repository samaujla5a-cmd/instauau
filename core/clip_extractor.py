...
Return ONLY valid JSON array."""
    ...
    try:
        ...
        logger.warning("clip_extractor: AI returned invalid JSON — using evenly-spaced fallback clips")
        clips = [
            {"clip_number": i+1, "start_sec": audio_duration/5*(i+1),
             "end_sec": audio_duration/5*(i+1)+45, "section": "hook",
             "title": f"Clip {i+1}", "caption_hook": concept.get("hook","")[:60],
             "why_viral": "auto", "energy_level": "high"}
            for i in range(4)
        ]
