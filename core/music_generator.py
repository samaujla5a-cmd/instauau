...
def _poll(task_id: str) -> dict:
    ...
    while waited < MAX_WAIT_SECS:
        ...
        data   = resp.json()
        d      = data.get("data") or {}
        # kie.ai Suno may use "state" OR "status" — check both
        status = d.get("state") or d.get("status", "PENDING")
        ...
        # GENERATE_AUDIO_FAILED is a terminal failure state from kie.ai Suno
        if status in ("ERROR", "FAILED", "TIMEOUT", "GENERATE_AUDIO_FAILED",
                      "error", "failed", "timeout", "generate_audio_failed"):
            raise RuntimeError(f"Suno generation failed (status={status}): {d.get('error', d.get('failMsg', ''))}")
...
def generate_song(concept: dict, suno_prompt: str = "") -> str:
    ...
    Updates concept in-place with lyric_timestamps + cover_image_url
    (intentional side-effects, now clearly documented).
    Returns local MP3 path.

    NOTE: this function mutates the passed concept dict to add
    lyric_timestamps and cover_image_url so the caller's video pipeline
    can use them. Pass a copy if you need the original unchanged.
    """
    ...
    # Clear any stale timestamps from a previous attempt before writing new ones
    concept.pop("lyric_timestamps", None)
    concept.pop("cover_image_url", None)
