...
def create_brainrot_video(content: dict, creature_image_path: str | None, duration: int = 30) -> str:
    ...
    Single robust FFmpeg command. All inputs declared before any output options.
    """
    ...
    # Ensure we have an image (real or fallback)
    has_image = bool(creature_image_path and os.path.exists(creature_image_path))
    if not has_image:
        try:
    ...
    def _run_cmd(use_image: bool, use_text: bool, use_voice: bool, output: str) -> int:
        """Build and run one FFmpeg attempt. Returns file size (0 = failed)."""
        cmd = ["ffmpeg", "-y"]

        # ── ALL INPUTS FIRST ────────────────────────────────────────────
        if use_image and has_image:
            # -r 30 on the input forces the still JPEG to produce frames at 30fps
            cmd += ["-loop", "1", "-r", "30", "-t", str(duration), "-i", creature_image_path]
        else:
            cmd += ["-f", "lavfi", "-t", str(duration),
                    "-i", f"color=c=0x1a0a00:size=1080x1920:rate=30"]

        if use_voice and has_voice:
            cmd += ["-i", voice_path]

        # ── ALL OUTPUT OPTIONS AFTER ALL INPUTS ─────────────────────────
        cmd += ["-map", "0:v"]
        ...
        if sz < 10_000:
            logger.warning(f"  attempt failed ({sz}b): {r.stderr.decode()[-200:]}")
            if Path(output).exists():
                os.remove(output)
            return 0
        return sz

    # Attempt 1: image + text + voice (full)
    sz = _run_cmd(use_image=True,  use_text=True,  use_voice=True,  output=out_path)
    if sz:
        logger.info(f"✅ Brainrot video: {out_path} ({sz // 1024}KB)")
        return out_path

    # Attempt 2: image + no text + voice
    sz = _run_cmd(use_image=True,  use_text=False, use_voice=True,  output=out_path)
    if sz:
        logger.info(f"✅ Brainrot video (no text): {out_path} ({sz // 1024}KB)")
        return out_path

    # Attempt 3: color background + text + voice (no image dependency at all)
    sz = _run_cmd(use_image=False, use_text=True,  use_voice=True,  output=out_path)
    if sz:
        logger.info(f"✅ Brainrot video (color bg): {out_path} ({sz // 1024}KB)")
        return out_path

    # Attempt 4: bare minimum — color + no text + no voice
    sz = _run_cmd(use_image=False, use_text=False, use_voice=False, output=out_path)
    if sz:
        logger.info(f"✅ Brainrot video (bare): {out_path} ({sz // 1024}KB)")
        return out_path

    raise RuntimeError(f"All 4 brainrot video attempts failed for {creature_name}")
