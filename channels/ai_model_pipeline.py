...
# kie.ai Market API endpoints (updated May 2026)
# Image: Flux Kontext generate endpoint
# Video: Seedance via market endpoint
# Task poll: unified /api/v1/jobs/recordInfo for market tasks
KIE_IMAGE_ENDPOINTS = [
    "https://api.kie.ai/api/v1/flux-kontext/generate",   # Flux Kontext (current)
    "https://api.kie.ai/api/v1/flux/generate",            # old Flux (fallback)
    "https://api.kie.ai/api/v1/image/flux/generate",      # legacy (last resort)
]
KIE_VIDEO_URL    = "https://api.kie.ai/api/v1/market/bytedance/seedance-2-fast"
KIE_TASK_IMG_URL = "https://api.kie.ai/api/v1/jobs/recordInfo"
KIE_TASK_VID_URL = "https://api.kie.ai/api/v1/jobs/recordInfo"
...
def _poll_image(task_id: str) -> str:
    ...
    while waited < MAX_WAIT:
        ...
        # Market API uses "state"; old API used "status" — support both
        ...
        if status == "success" or status == "SUCCESS":
            # Market API: resultJson is a JSON string
        ...
        if status in ("error", "ERROR", "failed", "FAILED"):
            raise RuntimeError(f"Image generation failed: {data.get('failMsg', data.get('error', data))}")
...
def _poll_video(task_id: str) -> str:
    ...
    while waited < MAX_WAIT:
        ...
        if status == "success" or status == "SUCCESS":
            ...
            return url
        if status in ("error", "ERROR", "failed", "FAILED"):
            raise RuntimeError(f"Video generation failed: {data.get('failMsg', data.get('error', data))}")
...
def generate_model_image(content: dict, session: str) -> str:
    ...
    for model_name in ["flux-kontext-pro", "flux-kontext-dev", "flux-dev"]:
        # Flux Kontext API uses a different payload structure
        if "kontext" in model_name:
            payload = {
                "callBackUrl": "https://example.com/callback",
                "input": {
                    "prompt":       image_prompt[:500],
                    "aspect_ratio": "4:5",   # closest to 1080x1350
                },
            }
        else:
            payload = {
...
def generate_model_video(image_path: str, content: dict, session: str) -> str:
    ...
    for model_name in ["seedance-2.0-fast", "seedance-1.5-pro", "seedance-2.0"]:
        # Map model names to their market API endpoint paths
        model_endpoint_map = {
        ...
        video_url = model_endpoint_map.get(model_name, KIE_VIDEO_URL)
        resp = requests.post(
            video_url,
            headers=_kie_headers(),
            json={
        ...
        if resp.status_code == 200:
            resp_data = resp.json()
            # Market API: data.taskId directly; old API: data.taskId nested
