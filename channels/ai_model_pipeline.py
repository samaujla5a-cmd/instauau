...
def create_reel_from_image(image_path: str, content: dict, duration: int = 15) -> str:
    ...
    # Attempt 1: image + drawtext + audio
    cmd1 = ["ffmpeg", "-y", "-loop", "1", "-i", image_path]
    if voice_path:
        cmd1 += ["-i", voice_path, "-map", "0:v", "-map", "1:a",
                 "-c:a", "aac", "-b:a", "128k"]
    cmd1 += [
    ...
    if voice_path:
        cmd2 += ["-i", voice_path, "-map", "0:v", "-map", "1:a", "-c:a", "aac"]
    cmd2 += [
        "-vf", "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920",
        "-t", str(duration),
        "-c:v", "libx264", "-preset", "fast",
    ...
    logger.info(f"Attempt 3 result: {sz}b")
    return out_path
